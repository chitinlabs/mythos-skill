...
```bash
# Windows
install/init.ps1 --user           # 用户级（所有项目）
install/init.ps1 D:\Work\MyGame   # 项目级
install/init.ps1 --both           # 两者都装
...
> **安装脚本会修改什么：** 把 skill 文件（SKILL.md、references/、examples/、tests/）复制到 `~/.claude/skills/mythos/`（用户级）或 `<project>/.claude/skills/mythos/`（项目级），并向同级的 `CLAUDE.md` **追加一段 "Reasoning Protocol" 块**——这会改变 Claude 路由复杂问题的方式。安装脚本不是 marketplace 包的一部分，只在本仓库提供，方便直接 git clone 的用户一键安装。
...
```
.claude/skills/mythos/                # marketplace 产物（跨 agent 通用）
...
│   ├── examples.md                  # 注释示例（SKILL.md 按需 lazy-load）
│   └── mythos-init.md               # 安装脚本注入到 CLAUDE.md 的协议片段
├── examples/                        # marketplace 展示用例
└── tests/                           # marketplace 测试文档（人工校准）

install/                              # 仅源仓库内工具（不打入 marketplace 包）
├── init.ps1 / init.sh               # 安装到其他项目或用户级 ~/.claude/CLAUDE.md
└── calibrate.ps1 / calibrate.sh     # 5 个标定用例的交互式跑测
```

## 校准

`install/calibrate.ps1` / `bash install/calibrate.sh` 提供 5 个标定用例（涵盖技术决策、战略权衡、缓存设计、流程改进、伦理争议），逐题交互式验证三种模式的结构性属性（lens 路径、轮数、并行 vs 串行 dispatch），生成时间戳报告 `calibration-report-YYYYMMDD-HHMM.md`。
